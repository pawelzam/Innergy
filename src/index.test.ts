...
describe.each([[2020], [2021], [2022]])(
  "calcularePrice.blueray",
  (year: ServiceYear) => {
    test("It does not make sense to include the price of the Extra Blu-ray when the client did not choose a video recording", () => {
      const price = calculatePrice(["BlurayPackage"], year);
      expect(price.finalPrice).toBe(0);
    });

    test("It does make sense to include the price of the Extra Blu-ray when the client chosen a video recording", () => {
      const price = calculatePrice(["BlurayPackage", "VideoRecording"], year);
...
describe.each([[2020], [2021], [2022]])(
  "calcularePrice.twoDayEvent",
  (year: ServiceYear) => {
    test("It does not make sense to include the price of handling two-day event when the client did not choose video recording or photography during the wedding", () => {
      const price = calculatePrice(["TwoDayEvent"], year);
      expect(price.finalPrice).toBe(0);
    });

    test("It does make sense to include the price of handling two-day event when the client chosen video recording or photography during the wedding", () => {
      const price = calculatePrice(["TwoDayEvent", "VideoRecording"], year);
      expect(price.finalPrice).toBeGreaterThan(0);
    });
  }
);
